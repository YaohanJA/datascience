...
def groupSumClump(start, nums, target):
  ...
  if groupSumClump(start + count, nums, target - count*nums[start]):
    return True
...
def splitArrayHelper(nums, start, balance):
  if (start == len(nums)):   
    return (balance == 0)
  if splitArrayHelper(nums, start+1 , balance+nums[start]):
    return True
  return splitArrayHelper(nums, start+1, balance-nums[start])
...
def splitOdd10Helper(nums, start, g1, g2):
  if (start == len(nums)):
    return ((g1 % 2 ==1 and g2 % 10 ==0) or (g2 % 2 ==1 and g1 % 10 ==0))
  if splitOdd10Helper(nums, start +1, g1 + nums[start], g2):
    return True
  return splitOdd10Helper(nums, start +1 , g1, g2 + nums[start])    
...
def split53Helper(nums, start, balance):
  if (start == len(nums)):
    return (balance == 0)
  if (nums[start] % 5 == 0):
    return split53Helper(nums, start +1, balance + nums[start])
  if (nums[start] % 3 == 0):
    return split53Helper(nums, start +1, balance - nums[start])
  if split53Helper(nums, start +1, balance + nums[start]):
    return True
  return split53Helper(nums, start +1, balance - nums[start])
